...
const HeroBanner: React.FC = () => {
  ...
  return (
    <section className="relative w-full h-screen overflow-hidden">
      ...
      {/* Subtle overlay to enhance text readability on varying background images */}
      <div className="absolute inset-0 bg-white/40 md:bg-white/30 lg:bg-white/20"></div>

      {/* Red Stylized Shape Container */}
      <div className="absolute inset-0 flex items-center justify-center p-4 md:p-8">
        <motion.div
          className="relative w-full h-full max-w-[1200px] max-h-[800px] flex items-center justify-start pointer-events-none" // Adjust max-width/height as needed
          initial={{ opacity: 0, x: -100 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 1, delay: 0.3 }}
        >
          {/* SVG for the irregular red shape */}
          <svg
            className="absolute top-1/2 left-1/2 -translate-x-[40%] -translate-y-1/2 w-full h-[120%] sm:h-[110%] md:h-[100%] lg:h-[90%] xl:h-[80%]" // Adjusted size and position
            viewBox="0 0 1000 800" // Define a viewBox suitable for the shape's aspect ratio
          ...
          >
            {/* This path attempts to approximate the shape. For a perfect match, you'd trace it in a graphics editor and export the path. */}
            <path
              d="M 0,0 C 300,50 600,0 800,100 C 1000,200 950,500 800,600 C 650,700 400,750 200,650 C 50,550 0,300 0,0 Z"
              fill="#DC2626" /* Tailwind's red-600 */
            />
          </svg>

          {/* Content (Text and Play Button) - positioned relative to the overall banner container */}
          <div className="absolute z-10 text-white text-center w-full px-4 sm:px-8 left-[40%] md:left-[35%] lg:left-[30%] -translate-x-1/2 -translate-y-1/2 top-1/2">
            <motion.h1
      ...
      {/* Fixed Social Media Icons (unchanged from previous) */}
      ...
      {/* Fixed Settings Icon (unchanged from previous) */}
      ...
      {/* Slider Navigation Arrows (unchanged from previous) */}
      ...
      {/* Video Modal (unchanged from previous) */}
      <AnimatePresence>
        {isModalOpen && (
          <motion.div
