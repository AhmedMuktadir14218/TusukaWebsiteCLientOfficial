...
const HeroBanner: React.FC = () => {
  ...
  return (
    <section className="relative w-full h-screen overflow-hidden">
      ...
      {/* Subtle overlay (optional, depends on how mix-blend-mode looks) */}
      {/* <div className="absolute inset-0 bg-white/40 md:bg-white/30 lg:bg-white/20"></div> */}

      {/* Red Stylized Shape Container */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <motion.div
          className="absolute bg-red-600 shadow-2xl origin-bottom-left" // Added shadow-2xl for depth, and origin for transformation
          initial={{ scaleX: 0, opacity: 0, x: '-50%' }} // Animate from left, scaled down
          animate={{ scaleX: 1, opacity: 1, x: '0%' }}
          transition={{ type: "spring", stiffness: 100, damping: 10, delay: 0.5 }}
          style={{
            width: 'clamp(500px, 120vw, 1200px)', // Make it very wide to cover the necessary area
            height: 'clamp(400px, 90vh, 900px)', // And quite tall
            left: '0%', // Start from the left edge of its parent
            top: '50%', // Align center vertically
            transform: 'translateY(-50%) translateX(-40%)', // Adjust position to bring it inward
            mixBlendMode: 'multiply', // Apply the blend mode from your CSS
            // The specific border-radius to create the desired half-circle/ellipse
            borderBottomRightRadius: '100%',
            borderTopLeftRadius: '0%', // Not specified in your CSS, but inferred from image
            borderTopRightRadius: '0%',
            borderBottomLeftRadius: '50%', // This is crucial for the curved bottom left
          }}
        >
             {/* The content will be placed absolutely within the overall banner section,
                 not directly within the red shape div, to give us more control over its position. */}
        </motion.div>
      </div>

      {/* Content (Text and Play Button) - positioned relative to the overall banner container */}
      {/* Adjusted positioning to be on top of the red shape, which is now on the left */}
      <div className="absolute z-10 text-white text-center w-full px-4 sm:px-8 left-1/2 -translate-x-[40%] -translate-y-1/2 top-1/2 md:max-w-xl lg:max-w-2xl mx-auto pointer-events-none">
        <motion.h1
