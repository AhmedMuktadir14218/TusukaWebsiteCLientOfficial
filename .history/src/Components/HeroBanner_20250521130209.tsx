  ...
  return (
    <section className="relative w-full h-screen overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center z-10">
        <motion.div
          className="relative bg-red-600 shadow-xl flex flex-col items-center justify-center p-8 sm:p-10 md:p-12 lg:p-16" // Adjusted padding for spacing
          initial={{ scale: 0.8, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ type: "spring", stiffness: 100, damping: 10, delay: 0.5 }}
          style={{
            // Position and size this div to be the red background for the text
            width: 'clamp(300px, 70vw, 750px)', // Max-width based on image
            height: 'clamp(250px, 40vh, 450px)', // Max-height
            borderRadius: '50% / 100% 100% 0 0', // This creates the rounded top and flat bottom
            mixBlendMode: 'multiply', // Apply the blend mode
            // Adjust transform to position it correctly on the screen
            transform: 'translate(-10%, -10%)', // Shift left and up slightly as per image
            pointerEvents: 'none', // Allow clicks to pass through
          }}
        >
          {/* Content (Video Play Icon and Text) - positioned inside the red background */}
          {/* The content itself also needs to be slightly responsive */}
          <div className="relative text-white text-center w-full max-w-lg mx-auto" style={{ pointerEvents: 'auto' }}> {/* Re-enable pointer events for the button */}
            {/* Video Play Icon - positioned above the text, centered */}
            <motion.button
              className="bg-white text-red-600 p-5 sm:p-6 rounded-full shadow-lg hover:scale-110 transition-transform duration-300 mb-6 sm:mb-8" // mb for spacing
            ...
            <motion.p
              className="text-sm sm:text-base md:text-lg max-w-xl mx-auto"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
